...
```
├── apply-quota.sh        # 운영 적용 — cgroup v1 (r7, g5, head)
├── rollback-quota.sh     # 롤백 — cgroup v1
├── verify-quota.sh       # 상태 확인 — cgroup v1
├── apply-quota-v2.sh     # 운영 적용 — cgroup v2 (dgx-a100)
├── rollback-quota-v2.sh  # 롤백 — cgroup v2
├── verify-quota-v2.sh    # 상태 확인 — cgroup v2
└── README.md
```
...
```bash
# cgroup v1 서버 (r7, g5, head) — SSH 세션 2개 이상 확보 후
sudo bash apply-quota.sh

# cgroup v2 서버 (dgx-a100) — SSH 세션 2개 이상 확보 후
sudo bash apply-quota-v2.sh
```
...
# cgroup v2
sudo bash rollback-quota-v2.sh
```
